...
public class DashboardController implements Initializable {
    ...
    private void getAnchorPane(AnchorPane source,Color start,Color end) {
        if (source.getId().equalsIgnoreCase(totalBook3dAnchor.getId()) || source.getId().equalsIgnoreCase(totalBookRoundAnchor.getId())) {
           fillTransition(totalBook3dAnchor,start,end);
           fillTransition(totalBookRoundAnchor,start,end);
        } else if (source.getId().equalsIgnoreCase(issue3dAnchor.getId()) || source.getId().equalsIgnoreCase(issueRoundAnchor.getId())) {
            fillTransition(issue3dAnchor,start,end);
            fillTransition(issueRoundAnchor,start,end);
        } else if (source.getId().equalsIgnoreCase(returnBook3dAnchor.getId()) || source.getId().equalsIgnoreCase(returnBookRoundAnchor.getId())){
    ...
    private void selectReport(String id){
        ...
        dataReportViewer.setTopAnchor(root, 0.0);
        dataReportViewer.setRightAnchor(root, 0.0);
        dataReportViewer.setBottomAnchor(root, 0.0);
        dataReportViewer.setLeftAnchor(root, 0.0);
    }
}
